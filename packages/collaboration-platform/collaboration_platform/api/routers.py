...
async def create_household(
    req: CreateHouseholdRequest, svc: HouseholdService = Depends(get_household_service)
):
...
async def add_member(
    ...
    svc: HouseholdService = Depends(get_household_service),
):
...
async def register_advisor(
    req: RegisterAdvisorRequest, svc: AdvisorService = Depends(get_advisor_service)
):
...
async def create_delegation(
    req: DelegateRequest, svc: DelegationService = Depends(get_delegation_service)
):
    return await svc.delegate_authority(
        req.delegator_user_id,
        req.delegatee_user_id,
        req.scope,
        req.household_id,
        req.expires_at,
    )
...
async def create_shared_space(
    ...
    svc: WorkspaceSharingService = Depends(get_workspace_service),
):
