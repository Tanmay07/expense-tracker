...
class HouseholdService:
    ...
    async def create_household(self, name: str, owner_id: str) -> Household:
        db_model = HouseholdModel(
            id=str(uuid.uuid4()), name=name, settings={"default_currency": "USD"}
        )
    ...
    async def add_member(
        self, household_id: str, user_id: str, role: MemberRole
    ) -> HouseholdMember:
        member_model = HouseholdMemberModel(
            id=str(uuid.uuid4()), household_id=household_id, user_id=user_id, role=role
        )
        await self.repo.add_member(member_model)
        return HouseholdMember(
            id=member_model.id, household_id=household_id, user_id=user_id, role=role
        )
...
class AdvisorService:
    ...
    async def register_advisor(
        self, user_id: str, specialty: str, firm_name: Optional[str] = None
    ) -> Advisor:
...
class WorkspaceSharingService:
    ...
    async def create_shared_workspace(
        self, name: str, owner_id: str, household_id: Optional[str] = None
    ) -> SharedWorkspace:
...
class SharedMissionService:
    async def create_mission(
        self, workspace_id: str, title: str, description: str, owners: List[str]
    ) -> SharedMission:
