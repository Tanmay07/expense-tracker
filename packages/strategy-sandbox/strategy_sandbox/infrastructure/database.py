...
class SandboxRunModel(Base):
    ...
    )  # Refers to MarketplaceAsset or external artifact
    profile_id = Column(
        String, ForeignKey("sandbox_validation_profiles.id"), nullable=False
    )

    status = Column(
        String, nullable=False, default="PENDING"
    )  # PENDING, IN_PROGRESS, PASSED, FAILED, TIMEOUT
    ...
    fitness_score = relationship(
        "FitnessScoreModel",
        back_populates="run",
        uselist=False,
        cascade="all, delete-orphan",
    )
    prompt_validation = relationship(
...
class SandboxCertificationModel(Base):
    ...
    certification_level = Column(
        String, nullable=False
    )  # EXPERIMENTAL, INTERNAL, CERTIFIED, ENTERPRISE_CERTIFIED, REGULATED
