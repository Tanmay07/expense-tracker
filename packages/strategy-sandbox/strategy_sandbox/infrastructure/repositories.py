...
from strategy_sandbox.infrastructure.database import (
    ValidationProfileModel, SandboxRunModel, BenchmarkRecordModel,
    FitnessScoreModel, PromptValidationModel, ReplaySnapshotModel,
    SandboxCertificationModel
)
from strategy_sandbox.domain.models import (
    ValidationProfile, SandboxRun, BenchmarkRecord, FitnessScore,
    PromptValidation, ReplaySnapshot, SandboxCertification
)
...
class ValidationProfileRepository(BaseRepository):
    def get_by_id(self, profile_id: str) -> Optional[ValidationProfile]:
        result = self.session.execute(
            select(ValidationProfileModel).where(ValidationProfileModel.id == profile_id)
        ).scalar_one_or_none()
    ...
    def list_profiles(self, limit: int = 50) -> List[ValidationProfile]:
        results = self.session.execute(select(ValidationProfileModel).limit(limit)).scalars().all()
...
class SandboxRunRepository(BaseRepository):
    ...
    def list_runs(self, asset_id: Optional[str] = None, limit: int = 50) -> List[SandboxRun]:
    ...
    def save(self, domain: SandboxRun) -> SandboxRun:
        ...
        dump = domain.model_dump(exclude={
            "profile", "fitness_score", "prompt_validation", 
            "benchmark", "replay_snapshot", "certification"
        })
...
class CertificationRepository(BaseRepository):
    def get_by_run_id(self, run_id: str) -> Optional[SandboxCertification]:
        result = self.session.execute(
            select(SandboxCertificationModel).where(SandboxCertificationModel.run_id == run_id)
        ).scalar_one_or_none()
    ...
    def save(self, domain: SandboxCertification) -> SandboxCertification:
        model = self.session.execute(
            select(SandboxCertificationModel).where(SandboxCertificationModel.id == domain.id)
        ).scalar_one_or_none()
        if not model:
