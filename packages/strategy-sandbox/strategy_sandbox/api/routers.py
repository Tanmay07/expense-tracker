...
def initiate_run(
    dto: SandboxRunCreate, svc: StrategySandboxService = Depends(get_sandbox_service)
) -> SandboxRun:
    try:
...
def list_runs(
    asset_id: Optional[str] = None, repo: SandboxRunRepository = Depends(get_run_repo)
) -> List[SandboxRun]:
...
def get_run(
    run_id: str, repo: SandboxRunRepository = Depends(get_run_repo)
) -> SandboxRun:
...
def calculate_fitness(
    run_id: str, svc: FitnessScoringService = Depends(get_fitness_service)
) -> FitnessScore:
...
def execute_benchmark(
    run_id: str, svc: BenchmarkService = Depends(get_benchmark_service)
) -> BenchmarkRecord:
...
def grant_certification(
    run_id: str, level: str, svc: CertificationService = Depends(get_cert_service)
) -> SandboxCertification:
    try:
