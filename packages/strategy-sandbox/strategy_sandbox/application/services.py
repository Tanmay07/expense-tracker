...
class StrategySandboxService:
    def __init__(
        self, run_repo: SandboxRunRepository, profile_repo: ValidationProfileRepository
    ):
    ...
    def initiate_run(self, dto: SandboxRunCreate) -> SandboxRun:
        ...
        run = SandboxRun(
            ...
            status="PENDING",
            current_stage=profile.required_stages[0]
            if profile.required_stages
            else None,
            stage_results_json={},
...
class CertificationService:
    def __init__(
        self, cert_repo: CertificationRepository, run_repo: SandboxRunRepository
    ):
