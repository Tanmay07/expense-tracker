...
from governance_platform.api.dependencies import (
    get_policy_service, get_trust_service, get_maturity_service,
    get_ai_gov_service, get_evidence_service, get_workflow_service, get_assurance_service
)
...
def record_evidence(
    ...
    return svc.record_evidence(asset_id, req.evidence_type, req.raw_payload, req.signer_id)
...
def transition_workflow(
    ...
    return svc.transition_state(asset_id, req.workflow_type, req.new_state, req.comments)

@router.post("/assurance/{asset_id}")
def run_continuous_assurance(
    asset_id: str,
    svc: AssuranceService = Depends(get_assurance_service)
):
