...
class EvidenceLedgerModel(Base):
    ...
    id = Column(String, primary_key=True)
    asset_id = Column(String, nullable=False, index=True)
    evidence_type = Column(String, nullable=False) # e.g., SANDBOX_PASS, MANUAL_APPROVAL
    ...
    previous_ledger_id = Column(String, ForeignKey("gov_evidence_ledger.id"), nullable=True)
...
class WorkflowStateModel(Base):
    ...
    current_state = Column(String, nullable=False, default="DRAFT") # DRAFT, REVIEW, APPROVED, REJECTED
