...
class MissionRepository(BaseRepository):
    def get_by_id(self, mission_id: str) -> Optional[Mission]:
        record = (
            self.db.query(MissionModel).filter(MissionModel.id == mission_id).first()
        )
        if not record:
            return None
        ...
        records = (
            ...
            .filter(
                ...
                MissionModel.status.in_(
                    [MissionStatusEnum.CREATED, MissionStatusEnum.UPDATED]
                ),
            )
            .order_by(MissionModel.urgency_score.desc())
            .all()
        )
