...
def get_missions(
    user_id: str, service: MissionControlService = Depends(get_mission_control_service)
):
...
def detect_opportunities(
    user_id: str, service: OpportunityService = Depends(get_opportunity_service)
):
    return service.detect_opportunities(user_id)


@router.post("/risks/detect", response_model=List[Risk])
def detect_risks(user_id: str, service: RiskService = Depends(get_risk_service)):
