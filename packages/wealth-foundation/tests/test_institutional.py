...
def test_wealth_institutional_step1():
    ...
    cust_res = client.post("/api/v1/foundation/custodians", json={
        "name": "JPMorgan Chase",
        "custodian_type": "BANK",
    ...
    # 2. Link Broker
    broker_res = client.post("/api/v1/foundation/brokers", json={
    ...
    custody_res = client.post(f"/api/v1/foundation/brokers/{broker_id}/custody", json={
    ...
    # 4. Invalid Broker
    bad_custody = client.post("/api/v1/foundation/brokers/invalid-broker/custody", json={
        "asset_id": "holding-nvda-123",
        "quantity": 50.0
    }, headers=headers)
...
def test_wealth_institutional_step2():
    ...
    # 1. Generate Tax Lot
    lot_res = client.post("/api/v1/foundation/tax-lots", json={
        "asset_id": "holding-nvda-123",
    ...
    wp_res = client.post("/api/v1/foundation/policies/wealth", json={
        "household_id": "hh-789",
        "name": "Max Equity 80%",
    ...
    pp_res = client.post("/api/v1/foundation/policies/portfolio", json={
...
def test_wealth_institutional_step3():
    ...
    # 1. Grant Permission
    perm_res = client.post("/api/v1/foundation/permissions", json={
        "household_id": "hh-789",
    ...
    # 2. Generate Snapshot
    snap_res = client.post("/api/v1/foundation/snapshots", json={
        "entity_id": "hh-789",
    ...
    replay_res = client.post("/api/v1/foundation/replay", json={
        "entity_id": "hh-789",
        "entity_type": "HOUSEHOLD",
