...
def test_wealth_foundation_step1():
    ...
    # 1. Create Household
    h_res = client.post(
        "/api/v1/foundation/households",
        json={"name": "Smith Family", "base_currency": "USD"},
        headers=headers,
    )
    ...
    # 2. Add Member (Spouse)
    m_res = client.post(
        f"/api/v1/foundation/households/{household_id}/members",
        json={"name": "Jane Smith", "role": "SPOUSE", "ownership_percentage": 50.0},
        headers=headers,
    )
    ...
    fx_res = client.get(
        "/api/v1/foundation/fx/convert?amount=1000&base=USD&target=EUR", headers=headers
    )
    ...
    bad_fx = client.get(
        "/api/v1/foundation/fx/convert?amount=1000&base=USD&target=JPY", headers=headers
    )
...
def test_wealth_foundation_step2():
    ...
    ca_res = client.post(
        "/api/v1/foundation/corporate-actions",
        json={"asset_symbol": "NVDA", "action_type": "SPLIT", "ratio": 10.0},
        headers=headers,
    )
    ...
    # 2. Link Goal Funding
    gf_res = client.post(
        "/api/v1/foundation/goal-funding",
        json={
