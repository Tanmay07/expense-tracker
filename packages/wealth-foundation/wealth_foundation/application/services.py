...
class HouseholdService:
    ...
    def add_member(self, household_id: str, name: str, role: str, ownership_percentage: float = 100.0, user_id: Optional[str] = None) -> FamilyMember:
...
class CorporateActionService:
    ...
    def announce_action(self, asset_symbol: str, action_type: str, ratio: Optional[float] = None, amount_per_share: Optional[float] = None) -> CorporateAction:
        ca = CorporateAction(
    ...
    def get_pending_actions(self, asset_symbol: str) -> List[CorporateAction]:
        return [ca for ca in self.actions.values() if ca.asset_symbol == asset_symbol and ca.status == "PENDING"]
...
class GoalFundingService:
    ...
    def link_asset_to_goal(self, goal_id: str, asset_id: str, allocation_percentage: float = 100.0, priority: int = 1) -> GoalFunding:
        gf = GoalFunding(
            goal_id=goal_id,
            asset_id=asset_id,
...
class ProviderRegistryService:
    ...
    def register_provider(self, name: str, provider_type: str, priority: int = 1, capabilities: List[str] = []) -> ProviderPlugin:
        pp = ProviderPlugin(
            name=name,
    ...
    def get_active_providers(self, provider_type: str) -> List[ProviderPlugin]:
        active = [p for p in self.providers.values() if p.provider_type == provider_type and p.is_active]
...
class ExplainabilityService:
    ...
    def log_explanation(self, recommendation_id: str, rule_applied: str, confidence_score: float, evidence: str) -> WealthExplanation:
...
class CustodianService:
    ...
    def register_custodian(self, name: str, custodian_type: str, capabilities: List[str] = []) -> Custodian:
        c = Custodian(
            name=name,
...
class BrokerService:
    ...
    def assign_custody(self, asset_id: str, broker_id: str, quantity: float) -> AssetCustody:
...
class TaxLotService:
    ...
    def generate_purchase_lot(self, asset_id: str, broker_id: str, quantity: float, unit_price: float) -> TaxLot:
        lot = TaxLot(
            asset_id=asset_id,
            broker_id=broker_id,
            quantity=quantity,
    ...
    def get_open_lots(self, asset_id: str, broker_id: str) -> List[TaxLot]:
        ...
        lots = [lot for lot in self.lots.values() if lot.asset_id == asset_id and lot.broker_id == broker_id and lot.status == "OPEN"]
...
class PolicyService:
    ...
    def create_wealth_policy(self, household_id: str, name: str, policy_type: str, parameters: dict) -> WealthPolicy:
        p = WealthPolicy(
            household_id=household_id,
            name=name,
    ...
    def create_portfolio_policy(self, portfolio_id: str, name: str, policy_type: str, parameters: dict) -> PortfolioPolicy:
        p = PortfolioPolicy(
...
class PermissionService:
    ...
    def grant_permission(self, household_id: str, user_id: str, role: str, granted_by: str) -> HouseholdPermission:
...
class SnapshotService:
    ...
    def generate_snapshot(self, entity_id: str, entity_type: str, snapshot_date: datetime, net_worth: float, asset_allocation: dict) -> WealthSnapshot:
        snap = WealthSnapshot(
            entity_id=entity_id,
...
class ReplayService:
    ...
    def build_replay_state(self, entity_id: str, entity_type: str, target_date: datetime, state_data: dict) -> ReplayState:
        rs = ReplayState(
