...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_analytics_experiments_name'), 'analytics_experiments', ['name'], unique=True)

    op.create_table('analytics_guardrails',
        sa.Column('id', sa.String(), nullable=False),
        ...
        sa.Column('auto_stop_conditions', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('updated_at', sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(['experiment_id'], ['analytics_experiments.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_analytics_guardrails_experiment_id'), 'analytics_guardrails', ['experiment_id'], unique=False)

    op.create_table('analytics_kpi_catalog',
        sa.Column('id', sa.String(), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_analytics_guardrails_experiment_id'), table_name='analytics_guardrails')
    op.drop_table('analytics_guardrails')
    op.drop_index(op.f('ix_analytics_experiments_name'), table_name='analytics_experiments')
