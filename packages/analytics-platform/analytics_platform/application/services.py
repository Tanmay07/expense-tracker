...
class ExperimentRegistryService:
    def __init__(
        ...
        guardrail_svc: StatisticalGuardrailService,
    ):
        self.repo = repo
...
class ExperimentAnalyticsService:
    def analyze_results(
        self, experiment_id: str, results_df: pl.DataFrame
    ) -> Dict[str, Any]:
        """
        Utilizes polars and scipy to calculate significance.
        """
        # Mock analysis
        return {"lift": 0.05, "p_value": 0.03, "significant": True}
...
class InsightEngineService:
    ...
    def generate_insight(
        self, title: str, description: str, severity: str = "INFO"
    ) -> Insight:
        ins = Insight(
