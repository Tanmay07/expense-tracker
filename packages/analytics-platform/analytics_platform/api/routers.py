...
from analytics_platform.application.services import (
    ...
    KPICatalogService, InsightEngineService, ExecutiveReportingService
)
from analytics_platform.api.dependencies import (
    get_experiment_service, get_guardrail_service,
    get_kpi_service, get_insight_service, get_reporting_service
)
...
def create_kpi(
    dto: KPICatalog,
    svc: KPICatalogService = Depends(get_kpi_service)
):
...
def calculate_mde(
    ...
    mde = svc.calculate_mde(req.baseline_conversion, req.sample_size, req.alpha, req.power)
...
def generate_insight(
    req: InsightReq,
    svc: InsightEngineService = Depends(get_insight_service)
):
