...
from analytics_platform.application.services import (
    ...
    KPICatalogService, ContinuousImprovementService,
    InsightEngineService, ExecutiveReportingService
)
...
def get_guardrail_service(repo: StatisticalGuardrailRepository = Depends(get_guardrail_repo)) -> StatisticalGuardrailService:
...
def get_kpi_service(repo: KPICatalogRepository = Depends(get_kpi_repo)) -> KPICatalogService:
    return KPICatalogService(repo)

def get_improvement_service() -> ContinuousImprovementService:
    return ContinuousImprovementService()

def get_insight_service(repo: InsightRepository = Depends(get_insight_repo)) -> InsightEngineService:
    return InsightEngineService(repo)

def get_reporting_service(repo: ExecutiveReportRepository = Depends(get_report_repo)) -> ExecutiveReportingService:
