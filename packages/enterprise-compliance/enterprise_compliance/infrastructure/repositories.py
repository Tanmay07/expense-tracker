...
from .database import (
    ...
    SuitabilityProfileModel,
    InvestmentConstraintModel,
    DecisionRecordModel,
)
...
class RiskProfileRepository(BaseRepository):
    def get_by_user_id(self, user_id: str) -> Optional[RiskProfile]:
        record = (
            self.db.query(RiskProfileModel)
            .filter(RiskProfileModel.user_id == user_id)
            .first()
        )
        if record:
...
class PolicyRepository(BaseRepository):
    def get_active_policies(self) -> List[CompliancePolicy]:
        records = (
            self.db.query(CompliancePolicyModel)
            .filter(CompliancePolicyModel.is_active)
            .all()
        )
...
class SuitabilityRepository(BaseRepository):
    def get_by_user_id(self, user_id: str) -> Optional[SuitabilityProfile]:
        record = (
            self.db.query(SuitabilityProfileModel)
            .filter(SuitabilityProfileModel.user_id == user_id)
            .first()
        )
        if record:
...
class ConstraintRepository(BaseRepository):
    def get_by_user_id(self, user_id: str) -> List[InvestmentConstraint]:
        records = (
            self.db.query(InvestmentConstraintModel)
            .filter(InvestmentConstraintModel.user_id == user_id)
            .all()
        )
...
class DecisionRepository(BaseRepository):
    def get_by_context_id(self, context_id: str) -> List[DecisionRecord]:
        records = (
            self.db.query(DecisionRecordModel)
            .filter(DecisionRecordModel.context_id == context_id)
            .all()
        )
