...
class DecisionMemoryRepository(BaseRepository):
    def create(self, memory_data: dict) -> DecisionMemoryResponse:
        record = DecisionMemoryModel(id=str(uuid.uuid4()), **memory_data)
        self.db.add(record)
        self.db.commit()
    ...
    def get_by_decision_id(self, decision_id: str) -> Optional[DecisionMemoryResponse]:
        record = (
            self.db.query(DecisionMemoryModel)
            .filter(DecisionMemoryModel.decision_id == decision_id)
            .first()
        )
        if record:
            return DecisionMemoryResponse.model_validate(record)
    ...
    def list_by_user(self, user_id: str) -> List[DecisionMemoryResponse]:
        records = (
            self.db.query(DecisionMemoryModel)
            .filter(DecisionMemoryModel.user_id == user_id)
            .all()
        )
...
class PatternRepository(BaseRepository):
    ...
    def list_by_user(self, user_id: str) -> List[PatternResponse]:
        records = (
            self.db.query(PatternModel).filter(PatternModel.user_id == user_id).all()
        )
...
class PersonalizationRepository(BaseRepository):
    def upsert(
        self, user_id: str, personalization_data: dict
    ) -> PersonalizationResponse:
        record = (
            self.db.query(PersonalizationModel)
            .filter(PersonalizationModel.user_id == user_id)
            .first()
        )
        if record:
            ...
            record = PersonalizationModel(
                id=str(uuid.uuid4()), user_id=user_id, **personalization_data
            )
            self.db.add(record)
        self.db.commit()
        self.db.refresh(record)
...
class PolicyDecisionCacheRepository(BaseRepository):
    ...
    def get_valid_cache(
        self, decision_id: str, policy_version: int
    ) -> Optional[PolicyCacheResponse]:
        record = (
            self.db.query(PolicyCacheModel)
            .filter(
    ...
    def invalidate(self, decision_id: str):
        records = (
            self.db.query(PolicyCacheModel)
            .filter(PolicyCacheModel.decision_id == decision_id)
            .all()
        )
        for r in records:
            r.is_valid = False
...
class PredictionRepository(BaseRepository):
    ...
    def get_latest_by_decision(self, decision_id: str) -> Optional[PredictionResponse]:
        record = (
            ...
            .filter(PredictionModel.decision_id == decision_id)
...
class FinancialDNARepository(BaseRepository):
    def upsert(self, user_id: str, dna_data: dict) -> FinancialDNAResponse:
        record = (
            self.db.query(FinancialDNAModel)
            .filter(FinancialDNAModel.user_id == user_id)
            .first()
        )
        if record:
            ...
            record = FinancialDNAModel(
                id=str(uuid.uuid4()), user_id=user_id, **dna_data
            )
            self.db.add(record)
        self.db.commit()
        self.db.refresh(record)
...
class BehaviorRepository(BaseRepository):
    ...
    def list_by_user(self, user_id: str) -> List[BehaviorResponse]:
        records = (
            self.db.query(BehaviorModel)
            .filter(BehaviorModel.user_id == user_id)
            .order_by(BehaviorModel.created_at.desc())
            .all()
        )
...
class ReplayRepository(BaseRepository):
    ...
    def get_by_session(self, session_id: str) -> Optional[ReplayResponse]:
        record = (
            self.db.query(ReplayModel)
            .filter(ReplayModel.session_id == session_id)
            .first()
        )
        if record:
            return ReplayResponse.model_validate(record)
