...
class OperationalTelemetrySDK:
    ...
    def recordEvent(
        self, category: str, source: str, payload: Dict[str, Any], trace_id: str = None
    ) -> Dict[str, Any]:
        data = {
            "category": category,
            "source": source,
    ...
    def recordAIEvent(self, source: str, metrics: Dict[str, Any]) -> Dict[str, Any]:
        data = {"source": source, "metrics": metrics}
        resp = self.client.post("/api/v1/ai-observability", json=data)
        resp.raise_for_status()
        return resp.json()

    def recordFinancialEvent(
        self, source: str, event_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        data = {"source": source, "event_data": event_data}
    ...
    def reportIncident(
        ...
        data = {"title": title, "severity": severity, "details": details}
