...
from observability_platform.infrastructure.repositories import (
    TelemetryRepository, MetricRepository, IncidentRepository,
    SLORepository, DashboardRepository
)
...
def get_telemetry_service(repo: TelemetryRepository = Depends(get_telemetry_repo)) -> TelemetryService:
    return TelemetryService(repo)

def get_metrics_service(repo: MetricRepository = Depends(get_metric_repo)) -> MetricsService:
    return MetricsService(repo)

def get_ai_obs_service(tel_svc: TelemetryService = Depends(get_telemetry_service)) -> AIObservabilityService:
    return AIObservabilityService(tel_svc)

def get_fin_obs_service(tel_svc: TelemetryService = Depends(get_telemetry_service)) -> FinancialObservabilityService:
    return FinancialObservabilityService(tel_svc)

def get_intelligence_service(repo: IncidentRepository = Depends(get_incident_repo)) -> OperationalIntelligenceService:
...
def get_dashboard_service(repo: DashboardRepository = Depends(get_dashboard_repo)) -> DashboardService:
