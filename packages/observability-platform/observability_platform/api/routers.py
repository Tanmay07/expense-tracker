...
def ingest_telemetry(
    dto: TelemetryEventCreate, svc: TelemetryService = Depends(get_telemetry_service)
):
...
def query_telemetry(
    category: str, svc: TelemetryService = Depends(get_telemetry_service)
):
...
def record_metric(
    dto: MetricRecordCreate, svc: MetricsService = Depends(get_metrics_service)
):
...
def track_ai_event(
    req: AITelemetryReq, svc: AIObservabilityService = Depends(get_ai_obs_service)
):
...
def register_slo(req: SLORegisterReq, svc: SLOService = Depends(get_slo_service)):
...
def create_dashboard(
    req: DashboardCreateReq, svc: DashboardService = Depends(get_dashboard_service)
):
