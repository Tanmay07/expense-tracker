...
class AgentDefinition(BaseModel):
    ...
    allowed_tools: List[str] = Field(default_factory=list, description="List of MCP tools or SDK functions allowed")
    memory_scope: MemoryScope = Field(default=MemoryScope.MISSION, description="Scope of agent's working memory")
    escalation_rules: List[str] = Field(default_factory=list, description="Rules for escalating to supervisor or human")
    approval_requirements: List[str] = Field(default_factory=list, description="Conditions requiring explicit human approval")
    policies: List[str] = Field(default_factory=list, description="List of policy IDs this agent must adhere to")
