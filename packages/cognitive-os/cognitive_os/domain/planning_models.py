...
class GoalDefinition(BaseModel):
    ...
    target_amount: Optional[float] = Field(None, description="Target financial amount if applicable")
    current_amount: Optional[float] = Field(None, description="Current progress amount")
    dependencies: List[str] = Field(default_factory=list, description="IDs of goals this goal depends on")
    status: str = Field(default="ACTIVE", description="Status: ACTIVE, COMPLETED, PAUSED, ABANDONED")
...
class MissionDefinition(BaseModel):
    ...
    priority: MissionPriority = Field(default=MissionPriority.ROUTINE, description="Execution priority")
    risk_level: str = Field(default="LOW", description="Assessed risk level (LOW, MEDIUM, HIGH)")
    financial_impact_estimate: float = Field(default=0.0, description="Estimated financial impact in dollars")
    dependencies: List[str] = Field(default_factory=list, description="Mission IDs that must complete first")
    context: Dict[str, Any] = Field(default_factory=dict, description="Contextual payload for execution")
    status: str = Field(default="PENDING", description="Status: PENDING, IN_PROGRESS, REVIEW, COMPLETED, FAILED")
