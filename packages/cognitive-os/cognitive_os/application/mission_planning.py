...
class MissionPlanningService:
    ...
    def generate_mission_for_goal(
        self, goal: GoalDefinition, context: Dict[str, Any]
    ) -> MissionDefinition:
        """
        ...
        priority = (
            MissionPriority.ELEVATED
            if goal.priority in ("HIGH", "CRITICAL")
            else MissionPriority.ROUTINE
        )
        ...
        mission = MissionDefinition(
            ...
            priority=priority,
            risk_level="LOW",
            financial_impact_estimate=goal.target_amount * 0.1
    ...
    def prioritize_missions(
        self, active_missions: List[MissionDefinition]
        ...
        def sort_key(m: MissionDefinition):
            priority_weight = {"URGENT": 3, "ELEVATED": 2, "ROUTINE": 1}.get(
                m.priority.value, 1
            )
            risk_weight = {"LOW": 1, "MEDIUM": 0.8, "HIGH": 0.5}.get(
                m.risk_level, 1
            )  # Prefer lower risk
