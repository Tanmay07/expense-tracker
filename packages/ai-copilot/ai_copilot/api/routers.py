...
def copilot_chat(
    request: ChatRequest, service: CopilotService = Depends(get_copilot_service)
):
    try:
...
def create_action_plan(
    request: PlanRequest, service: ActionPlanService = Depends(get_action_plan_service)
):
...
def request_simulation(
    ...
    return service.request_simulation(
        request.user_id, request.scenario, request.assumptions
    )
