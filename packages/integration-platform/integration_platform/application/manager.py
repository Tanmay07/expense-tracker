...
class ConnectorManagerService:
    ...
    """
    def __init__(self, registry: ConnectorRegistryService, ff_client: FeatureFlagClient = None):
        self.registry = registry
        self.ff_client = ff_client or FeatureFlagClient()
        # In production, uses a database to track installed connections per user

    async def install_connector(self, user_id: str, connector_id: str, config: Dict[str, Any]) -> str:
        """
        ...
        if not self.ff_client.is_enabled(f"integration.connector.{connector_id}.enabled", {"user_id": user_id}):
            raise PermissionError(f"Connector {connector_id} is currently disabled by feature flag.")
