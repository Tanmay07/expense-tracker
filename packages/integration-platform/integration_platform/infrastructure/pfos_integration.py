...
class ObservabilityClient:
    ...
    def record_sync_metric(
        self, connector_id: str, duration_ms: float, records_fetched: int
    ):
        # Mock implementation. In prod, emits custom metrics to OTel collector
        logger.info(
            f"METRIC [sync.completed]: connector={connector_id} duration={duration_ms}ms records={records_fetched}"
        )
