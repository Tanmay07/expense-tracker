...
from integration_platform.application.auth import AuthenticationService, MockAWSSecretsManager
...
from integration_platform.infrastructure.pfos_integration import FeatureFlagClient, GovernanceClient, ObservabilityClient
...
class MockPlaidConnector(ConnectorBase):
    ...
    def get_metadata(self) -> Any:
        from integration_platform.domain.models import ConnectorMetadata, ConnectorType, ConnectorStatus
...
async def test_end_to_end_efcip_flow():
    ...
    sync_result = await sync_service.trigger_sync(connection_id, "plaid_v1", user_id, sync_type="initial")
    ...
    context = SyncContext(connection_id=connection_id, user_id=user_id, last_sync_token=None, parameters={})
