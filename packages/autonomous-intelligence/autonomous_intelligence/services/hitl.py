...
class HITLService:
    ...
    async def determine_classification(self, agent: Agent, proposed_action: Dict[str, Any]) -> HITLClassification:
        logger.info(f"Determining HITL classification for agent {agent.id} action {proposed_action}")
    ...
    async def request_human_approval(self, agent_id: uuid.UUID, action_details: Dict[str, Any]) -> str:
