...
class GovernanceService:
    ...
    async def evaluate_policy_compliance(
        ...
        """
        logger.info(
            f"Evaluating policy compliance for agent {agent.id} against action {proposed_action}"
        )
        ...
        if tool_name not in agent.allowed_tools:
            logger.warning(
                f"Policy violation: Agent {agent.id} attempted to use forbidden tool {tool_name}"
            )
            return False
