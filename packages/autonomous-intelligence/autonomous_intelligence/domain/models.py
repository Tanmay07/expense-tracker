...
class Agent(Base):
    ...
    risk_classification: Mapped[RiskClassification] = mapped_column(
        SQLEnum(RiskClassification), nullable=False
    )
...
class Goal(Base):
    ...
    agent_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("afip_agents.id"), nullable=False
    )
...
class Mission(Base):
    ...
    goal_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("afip_goals.id"), nullable=True
    )
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="CREATED")
    plan: Mapped[Dict[str, Any]] = mapped_column(JSONB, default=dict)
    hitl_classification: Mapped[HITLClassification] = mapped_column(
        SQLEnum(HITLClassification), nullable=False
    )
...
class Plan(Base):
    ...
    mission_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("afip_missions.id"), nullable=True
    )
    horizon: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # DAILY, WEEKLY, MONTHLY, ANNUAL, MULTI_YEAR
...
class Memory(Base):
    ...
    memory_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # WORKING, EPISODIC, LONG_TERM, SHARED
    reference_id: Mapped[str] = mapped_column(
        String(255), nullable=True
    )  # pointer to Knowledge Graph or Timeline
    content: Mapped[Dict[str, Any]] = mapped_column(JSONB, nullable=False)
