...
def test_timeline_event_sourcing_and_replay():
    ...
    rep1 = client.get(
        f"/api/v1/timeline/replay/net_worth?target_timestamp={t1}", headers=headers
    )
    ...
    rep2 = client.get(
        f"/api/v1/timeline/replay/net_worth?target_timestamp={t2}", headers=headers
    )
    assert rep2.json()["reconstructed_net_worth"] == 3500.0

    # Replay at t3 (Should be 3500 + 500 = 4000)
    rep3 = client.get(
        f"/api/v1/timeline/replay/net_worth?target_timestamp={t3}", headers=headers
    )
