...
from decision_registry.domain.models import (
    Decision, DecisionType, DecisionCategory, DecisionScope,
    DecisionPriority, DecisionVersion, DecisionMetadata, DecisionProvenance
)
...
def test_decision_creation_and_retrieval():
    ...
    d = Decision(
        ...
        version_info=DecisionVersion(version=1, semantic_version="1.0.0", created_at=datetime.utcnow()),
        metadata=DecisionMetadata(source="AI_COACH", owner="SYSTEM", summary="Buy AAPL", narrative="AAPL is good"),
        provenance=DecisionProvenance(metrics_used=["AAPL_PE"], policies_used=["MAX_ALLOCATION"])
    )
