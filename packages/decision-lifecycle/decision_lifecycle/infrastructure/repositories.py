...
class LifecycleRepository(BaseRepository):
    def get_by_decision_id(self, decision_id: str) -> Optional[DecisionLifecycle]:
        record = (
            self.db.query(DecisionLifecycleModel)
            .filter(DecisionLifecycleModel.decision_id == decision_id)
            .first()
        )
        if not record:
            return None
...
class ExecutionPlanRepository(BaseRepository):
    def get_by_decision_id(self, decision_id: str) -> Optional[ExecutionPlan]:
        record = (
            self.db.query(ExecutionPlanModel)
            .filter(ExecutionPlanModel.decision_id == decision_id)
            .first()
        )
        if not record:
            return None
        ...
        steps = (
            [ExecutionStep(**s) for s in record.steps_json] if record.steps_json else []
        )
        milestones = (
            [Milestone(**m) for m in record.milestones_json]
            if record.milestones_json
            else []
        )
        rollback = (
            RollbackPlan(**record.rollback_plan_json)
            if record.rollback_plan_json
            else None
        )
    ...
    def save(self, plan: ExecutionPlan) -> ExecutionPlan:
        record = ExecutionPlanModel(
            ...
            rollback_plan_json=plan.rollback_plan.model_dump()
