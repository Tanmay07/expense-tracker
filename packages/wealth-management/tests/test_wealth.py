...
def test_wealth_management_step1():
    ...
    # 1. Create Portfolio
    p_res = client.post("/api/v1/wealth/portfolios", json={
    ...
    # 2. Deposit Cash
    dep_res = client.post(f"/api/v1/wealth/portfolios/{portfolio_id}/deposit?amount=10000.00", headers=headers)
    assert dep_res.status_code == 200
    assert dep_res.json()["cash_balance"] == 10000.00
    
    # 3. Buy Investment (Apple Stock)
    buy_res = client.post("/api/v1/wealth/investments/buy", json={
    ...
    bad_buy = client.post("/api/v1/wealth/investments/buy", json={
        "portfolio_id": portfolio_id,
        "asset_symbol": "TSLA",
        "asset_class": "EQUITY",
        "quantity": 100.0,
