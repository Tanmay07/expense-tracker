...
class ConversationRepository(BaseRepository):
    def get_by_id(self, conversation_id: str) -> Optional[Conversation]:
        record = self.db.query(ConversationModel).filter(ConversationModel.id == conversation_id).first()
        if not record:
            return None
        
        messages = self.db.query(MessageModel).filter(MessageModel.conversation_id == conversation_id).order_by(MessageModel.timestamp).all()
...
class MemoryRepository(BaseRepository):
    def get_by_user_id(self, user_id: str) -> List[MemoryItem]:
        records = self.db.query(MemoryItemModel).filter(MemoryItemModel.user_id == user_id).all()
        return [MemoryItem(**r.__dict__) for r in records]
        
    def save(self, memory: MemoryItem, embedding: Optional[List[float]] = None) -> MemoryItem:
...
class PromptRepository(BaseRepository):
    def get_by_name(self, name: str) -> Optional[PromptTemplate]:
        ...
        record = self.db.query(PromptTemplateModel).filter(PromptTemplateModel.name == name).order_by(PromptTemplateModel.version.desc()).first()
        if record:
...
class AgentRepository(BaseRepository):
    def get_by_name(self, name: str) -> Optional[AgentDefinition]:
        record = self.db.query(AgentDefinitionModel).filter(AgentDefinitionModel.name == name).first()
        if record:
