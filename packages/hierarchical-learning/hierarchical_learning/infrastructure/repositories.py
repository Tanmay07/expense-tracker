...
class RegionalLearningRepository(BaseRepository):
    def get_by_region_and_topic(
        self, region_id: str, topic: str
    ) -> Optional[RegionalLearning]:
        result = self.session.execute(
            select(RegionalLearningModel).where(
                and_(
                    RegionalLearningModel.region_id == region_id,
                    RegionalLearningModel.topic == topic,
                )
            )
        ).scalar_one_or_none()
...
class HouseholdLearningRepository(BaseRepository):
    def get_by_household_and_topic(
        self, household_id: str, topic: str
    ) -> Optional[HouseholdLearning]:
        result = self.session.execute(
            select(HouseholdLearningModel).where(
                and_(
                    HouseholdLearningModel.household_id == household_id,
                    HouseholdLearningModel.topic == topic,
                )
            )
        ).scalar_one_or_none()
...
class HouseholdConsensusRepository(BaseRepository):
    def get_by_household_and_topic(
        self, household_id: str, topic: str
    ) -> Optional[HouseholdConsensus]:
        result = self.session.execute(
            select(HouseholdConsensusModel).where(
                and_(
                    HouseholdConsensusModel.household_id == household_id,
                    HouseholdConsensusModel.conflict_topic == topic,
                )
            )
        ).scalar_one_or_none()
    ...
    def save(self, domain: HouseholdConsensus) -> HouseholdConsensus:
        model = self.session.execute(
            select(HouseholdConsensusModel).where(
                HouseholdConsensusModel.id == domain.id
            )
        ).scalar_one_or_none()
...
class PersonalLearningRepository(BaseRepository):
    ...
    ) -> Optional[PersonalLearning]:
        result = self.session.execute(
            select(PersonalLearningModel).where(
                and_(
                    PersonalLearningModel.user_id == user_id,
                    PersonalLearningModel.topic == topic,
                )
            )
        ).scalar_one_or_none()
        return PersonalLearning.model_validate(result) if result else None

    def find_similar_behavior(
        ...
        results = (
            self.session.execute(
                ...
                .order_by(
                    PersonalLearningModel.semantic_embedding.l2_distance(embedding)
                )
                .limit(limit)
            )
            .scalars()
...
class KnowledgePromotionRepository(BaseRepository):
            ...
            self.session.execute(
                select(KnowledgePromotionModel).where(
                    and_(
                        ...
                        KnowledgePromotionModel.promotion_status.in_(
                            ["OBSERVED", "CANDIDATE"]
                        ),
                    )
                )
    ...
    def save(self, domain: KnowledgePromotion) -> KnowledgePromotion:
        model = self.session.execute(
            select(KnowledgePromotionModel).where(
                KnowledgePromotionModel.id == domain.id
            )
        ).scalar_one_or_none()
