...
class GlobalLearningModel(Base):
    ...
    status = Column(
        String, default="VALIDATED"
    )  # CANDIDATE, VALIDATED, APPROVED, DEPRECATED
...
class HouseholdLearningModel(Base):
    ...
    member_permissions_json = Column(
        JSON
    )  # e.g. {"user_1": "READ", "user_2": "READ_WRITE"}
...
class PersonalLearningModel(Base):
    ...
    )  # pgvector for behavior matching
    last_updated = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class KnowledgePromotionModel(Base):
    ...
    promotion_status = Column(
        String, default="OBSERVED"
    )  # OBSERVED, CANDIDATE, APPROVED, PUBLISHED, REJECTED
