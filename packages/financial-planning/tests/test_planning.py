...
def test_financial_planning_engine():
    ...
    # 1. Create a Budget
    res1 = client.post(
        "/api/v1/planning/budgets",
        json={"name": "Groceries", "amount": 500.00, "period": "MONTHLY"},
        headers=headers,
    )
    ...
    res2 = client.post(
        f"/api/v1/planning/budgets/{budget_id}/spend?amount=600.00", headers=headers
    )
    ...
    res3 = client.post(
        ...
        json={"name": "Vacation Fund", "target_amount": 1000.00},
        headers=headers,
    )
    ...
    # 4. Fund Goal
    res4 = client.post(
        f"/api/v1/planning/goals/{goal_id}/fund?amount=1000.00", headers=headers
    )
