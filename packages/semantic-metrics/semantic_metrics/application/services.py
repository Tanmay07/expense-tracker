...
class CalculationGraphEngine:
    def __init__(self, lineage: LineageService):
        ...
        self.dag = {
            ...
            "income": {"depends_on": ["raw_ledger_credits"], "formula": "sum(credits)"},
            "expenses": {"depends_on": ["raw_ledger_debits"], "formula": "sum(debits)"},
        }
    ...
    def resolve_metric(self, metric_id: str, raw_data: Dict[str, float]) -> float:
        ...
        self.lineage.record_trace(
            query_id, metric_id, deps, ["postgres.ledger_entries"]
        )
