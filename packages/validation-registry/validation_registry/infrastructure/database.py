...
from sqlalchemy import (
    ...
    ForeignKey, ARRAY, Boolean, UniqueConstraint
)
...
class ArtifactRecordModel(Base):
    ...
    category = Column(String, nullable=False) # e.g. VALIDATION_REPORT, MONTE_CARLO_OUTPUT, REPLAY_PACKAGE
    ...
    status = Column(String, nullable=False, default="ACTIVE") # ACTIVE, DEPRECATED, ARCHIVED, PURGED
    ...
    lineage_sources = relationship("ArtifactLineageModel", foreign_keys="[ArtifactLineageModel.target_id]", back_populates="target")
    lineage_targets = relationship("ArtifactLineageModel", foreign_keys="[ArtifactLineageModel.source_id]", back_populates="source")
...
class ArtifactLineageModel(Base):
    ...
    relationship_type = Column(String, nullable=False) # DERIVED_FROM, BUNDLED_IN, VALIDATED_BY
    
    created_at = Column(DateTime, default=datetime.utcnow)
    
    source = relationship("ArtifactRecordModel", foreign_keys=[source_id], back_populates="lineage_targets")
    target = relationship("ArtifactRecordModel", foreign_keys=[target_id], back_populates="lineage_sources")
    
    UniqueConstraint('source_id', 'target_id', 'relationship_type', name='uq_lineage_edge')
...
class ReuseEvaluationModel(Base):
    ...
    id = Column(String, primary_key=True)
    target_artifact_id = Column(String, ForeignKey("var_artifact_records.id"), nullable=False)
