...
class OptimizationService:
    ...
    def generate_candidates(self, user_id: str, profile_data: Dict[str, Any]) -> List[DecisionCandidate]:
        """
        ...
        c1 = DecisionCandidate(
            ...
            overall_score=85.5,
            sub_scores=SubScores(financial_impact=90, risk=20, urgency=80, goal_alignment=85, liquidity_impact=-50)
        )
        ...
        c2 = DecisionCandidate(
            ...
            proposed_actions=[{"type": "save", "amount": 2500}, {"type": "pay_debt", "amount": 2500}],
            overall_score=82.0,
            sub_scores=SubScores(financial_impact=70, risk=10, urgency=50, goal_alignment=80, liquidity_impact=10)
        )
...
class OpportunityCostService:
    ...
    def calculate_cost(self, candidate_id: str) -> Optional[DecisionCandidate]:
        ...
        future_val = sum(a.get("amount", 0) for a in candidate.proposed_actions) * 1.05 ** 10
