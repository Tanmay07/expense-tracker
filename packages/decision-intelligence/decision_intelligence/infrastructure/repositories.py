...
class CandidateRepository(BaseRepository):
    def get_by_id(self, candidate_id: str) -> Optional[DecisionCandidate]:
        record = (
            self.db.query(DecisionCandidateModel)
            .filter(DecisionCandidateModel.id == candidate_id)
            .first()
        )
        if not record:
            return None
        ...
        opp_cost = (
            OpportunityCost(**record.opportunity_cost)
            if record.opportunity_cost
            else None
        )
        ...
        violations = (
            [ConstraintViolation(**v) for v in record.constraint_violations]
    ...
    def save(self, candidate: DecisionCandidate) -> DecisionCandidate:
        record = DecisionCandidateModel(
            ...
            if candidate.sub_scores
            else None,
            opportunity_cost=candidate.opportunity_cost.model_dump()
            if candidate.opportunity_cost
            else None,
            confidence=candidate.confidence.model_dump()
            if candidate.confidence
            else None,
            evidence=candidate.evidence.model_dump() if candidate.evidence else None,
            constraint_violations=[
                v.model_dump() for v in candidate.constraint_violations
            ],
