...
class PlatformManagementSDK:
    ...
    def certify_release(self, version: str) -> dict:
        response = requests.post(
            f"{self.base_url}/api/v1/platform/certification",
    ...
    def capture_baseline(self, version: str) -> dict:
        response = requests.post(
            f"{self.base_url}/api/v1/platform/baseline", params={"version": version}
        )
        response.raise_for_status()
        return response.json()
    ...
    def freeze_contracts(self, version: str) -> dict:
        response = requests.post(
            f"{self.base_url}/api/v1/platform/contracts/freeze",
            params={"version": version},
        )
        response.raise_for_status()
        return response.json()
    ...
    def freeze_documentation(self, version: str) -> dict:
        response = requests.post(
            f"{self.base_url}/api/v1/platform/documentation/freeze",
            params={"version": version},
        )
        response.raise_for_status()
        return response.json()
    ...
    def generate_release(
        self, version: str, cert_id: str, base_id: str, contract_id: str, doc_id: str
    ) -> dict:
        response = requests.post(
