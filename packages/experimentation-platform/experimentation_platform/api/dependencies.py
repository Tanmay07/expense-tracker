...
from experimentation_platform.application.services import (
    FeatureRegistryService, FeatureFlagService, TargetingService,
    ProgressiveDeliveryService, ExperimentService, AIExperimentService,
...
def get_feature_service(repo: FeatureRepository = Depends(get_feature_repo)) -> FeatureRegistryService:
...
def get_delivery_service(repo: RolloutRepository = Depends(get_rollout_repo)) -> ProgressiveDeliveryService:
    return ProgressiveDeliveryService(repo)

def get_experiment_service(repo: ExperimentRepository = Depends(get_experiment_repo)) -> ExperimentService:
    return ExperimentService(repo)

def get_ai_experiment_service(repo: ExperimentRepository = Depends(get_experiment_repo)) -> AIExperimentService:
    return AIExperimentService(repo)

def get_rollback_service(flag_repo: FeatureFlagRepository = Depends(get_flag_repo)) -> RollbackService:
    return RollbackService(flag_repo)

def get_analytics_service(repo: ExperimentAnalyticsRepository = Depends(get_analytics_repo)) -> ExperimentAnalyticsService:
