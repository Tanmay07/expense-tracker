...
def run_architecture_audit(req: ComponentRequest, db: Session = Depends(get_db)):
...
def run_security_assessment(req: ComponentRequest, db: Session = Depends(get_db)):
...
def run_performance_certification(
    req: PerformanceRequest, db: Session = Depends(get_db)
):
...
@router.post("/chaos")
def run_chaos_experiment(req: ChaosRequest, db: Session = Depends(get_db)):
...
@router.post("/cost")
def estimate_cost(req: ComponentRequest, db: Session = Depends(get_db)):
...
@router.post("/score")
def calculate_readiness_score(req: ScoreRequest, db: Session = Depends(get_db)):
...
def get_certification(version_tag: str, db: Session = Depends(get_db)):
    svc = get_production_readiness_service(db)
