...
class PlatformReadinessSDK:
    ...
    def runArchitectureAudit(self, component_name: str) -> Dict[str, Any]:
        resp = self.client.post("/api/v1/readiness/architecture", json={"component_name": component_name})
        resp.raise_for_status()
        return resp.json()

    def runSecurityAssessment(self, component_name: str) -> Dict[str, Any]:
        resp = self.client.post("/api/v1/readiness/security", json={"component_name": component_name})
        resp.raise_for_status()
        return resp.json()

    def runPerformanceCertification(self, endpoint: str) -> Dict[str, Any]:
        resp = self.client.post("/api/v1/readiness/performance", json={"endpoint": endpoint})
        resp.raise_for_status()
        return resp.json()

    def runChaosExperiment(self, scenario_name: str, target_component: str) -> Dict[str, Any]:
        resp = self.client.post("/api/v1/readiness/chaos", json={"scenario_name": scenario_name, "target_component": target_component})
        resp.raise_for_status()
        return resp.json()
    ...
    def calculateReadiness(self, version_tag: str) -> Dict[str, Any]:
        resp = self.client.post("/api/v1/readiness/score", json={"version_tag": version_tag})
        resp.raise_for_status()
        return resp.json()
