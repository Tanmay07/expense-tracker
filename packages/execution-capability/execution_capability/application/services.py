...
class RoutingService:
    ...
    def determine_route(
        ...
        route = ExecutionRoute(step_id=step_id, selected_capability_id=selected.id)

        # Enforce Automation Level constraints
        if (
            selected.automation_level == AutomationLevel.APPROVAL_REQUIRED
            or selected.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]
        ):
