...
class CapabilityRepository(BaseRepository):
    def get_by_id(self, capability_id: str) -> Optional[ExecutionCapability]:
        record = (
            self.db.query(CapabilityModel)
            .filter(CapabilityModel.id == capability_id)
            .first()
        )
        if not record:
            return None
        ...
        prereqs = (
            [ExecutionPrerequisite(**p) for p in record.prerequisites_json]
            if record.prerequisites_json
            else []
        )
    ...
    def list_all(self) -> List[ExecutionCapability]:
        ...
        for r in records:
            prereqs = (
                [ExecutionPrerequisite(**p) for p in r.prerequisites_json]
                if r.prerequisites_json
                else []
            )
            result.append(
                ExecutionCapability(
