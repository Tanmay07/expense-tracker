...
class ExecutionCapabilitySDK:
    ...
    def plan_execution_route(self, step_id: str, required_tags: List[str]) -> Optional[Dict[str, Any]]:
        resp = requests.post(f"{self.base_url}/routing", json={"step_id": step_id, "required_tags": required_tags})
        if resp.status_code == 404:
            return None
        resp.raise_for_status()
        return resp.json()

    def request_approval(self, execution_step_id: str, capability_id: str, requested_by: str) -> Dict[str, Any]:
        payload = {
