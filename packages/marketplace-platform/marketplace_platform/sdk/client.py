...
class MarketplaceSDK:
    ...
    def checkAiUsability(self, asset_id: str, ai_consumer: str) -> bool:
        resp = self.client.get(f"/marketplace/assets/{asset_id}/usability", params={"ai_consumer": ai_consumer})
        resp.raise_for_status()
        return resp.json()
    ...
    def deprecateAsset(self, asset_id: str, reason: str) -> Dict[str, Any]:
        resp = self.client.post(f"/marketplace/assets/{asset_id}/deprecate", params={"reason": reason})
        resp.raise_for_status()
        return resp.json()
        
    def certifyAsset(self, asset_id: str, certifier_id: str, tier: str) -> Dict[str, Any]:
        params = {"certifier_id": certifier_id, "tier": tier}
        resp = self.client.post(f"/marketplace/assets/{asset_id}/certify", params=params)
        resp.raise_for_status()
        return resp.json()
