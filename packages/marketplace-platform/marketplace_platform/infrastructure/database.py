...
class MarketplaceAssetModel(Base):
    ...
    capability_matrix = relationship(
...
class KnowledgeCapabilityMatrixModel(Base):
    ...
    id = Column(String, primary_key=True)
    asset_id = Column(
        String, ForeignKey("marketplace_assets.id"), nullable=False, unique=True
    )

    scope = Column(String, nullable=False)  # GLOBAL, REGIONAL, HOUSEHOLD, PERSONAL
    visibility = Column(
        String, nullable=False
    )  # PRIVATE, HOUSEHOLD, ORGANIZATION, PUBLIC
    sensitivity = Column(
        String, nullable=False
    ...
    ai_usability_json = Column(
        JSONB, nullable=False, default=dict
    )  # e.g. {"ai_coach": true, "digital_twin": false}
