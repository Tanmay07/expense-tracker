...
from marketplace_platform.infrastructure.database import (
    ...
    KnowledgeCapabilityMatrixModel,
    CertificationModel,
    AssetRankingModel,
)
from marketplace_platform.domain.models import (
    MarketplaceAsset,
    KnowledgeCapabilityMatrix,
    Certification,
    AssetRanking,
)
...
class MarketplaceAssetRepository(BaseRepository):
    ...
    def list_assets(
        ...
        query = select(MarketplaceAssetModel).where(
            MarketplaceAssetModel.status == status
        )
        if asset_type:
    ...
    def save(self, domain: MarketplaceAsset) -> MarketplaceAsset:
        ...
        if not model:
            ...
            dump = domain.model_dump(
                exclude={"capability_matrix", "certification", "ranking"}
            )
...
class KnowledgeCapabilityMatrixRepository(BaseRepository):
    def get_by_asset_id(self, asset_id: str) -> Optional[KnowledgeCapabilityMatrix]:
        result = self.session.execute(
            select(KnowledgeCapabilityMatrixModel).where(
                KnowledgeCapabilityMatrixModel.asset_id == asset_id
            )
        ).scalar_one_or_none()
    ...
    def save(self, domain: KnowledgeCapabilityMatrix) -> KnowledgeCapabilityMatrix:
        model = self.session.execute(
            select(KnowledgeCapabilityMatrixModel).where(
                KnowledgeCapabilityMatrixModel.id == domain.id
            )
        ).scalar_one_or_none()
    ...
    def get_top_assets(self, limit: int = 10) -> List[AssetRanking]:
        results = (
            self.session.execute(
                select(AssetRankingModel)
                .order_by(AssetRankingModel.overall_quality_score.desc())
                .limit(limit)
            )
            .scalars()
            .all()
